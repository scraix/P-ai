...
- 重构（chat-persistence）：会话 metadata 修改收敛到会话服务字段级入口，指定模型、计划模式、标题、未读数、工作区信息、归档状态、时间戳、当前 Todo、用户画像快照与记忆召回表先更新内存缓存再排队落盘；后台持久化支持 meta-only 队列，并在整会话写入时保留字段级 metadata，避免旧快照覆盖用户刚切换的会话模型。
